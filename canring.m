/*Original from the makers of:
https://github.com/edgarcosta/hilbertmodularforms
Eran Assaf, Angelica Babei, Ben Breen, Sara Chari, Edgar Costa,
Juanita Duque-Rosero, Avinash Kulkarni, Grant Molnar, Michael Musty,
Sam Schiavone, Shikhin Sethi, Samuel Tripp, and John Voight
...
[Allan Steel, June 2022:]
Since this algorithm actually extends the relations to a homogeneous
degree-d Groebner basis at each successive degree d, any new monomial
which is divisible by the leading monomial of a previous relation
(Groebner basis element) can be eliminated, so unnecessary relations
are avoided, making the matrices much smaller, so the algorithm is
very much faster.
*/
...
MinimumPrecision := function(N,k : Buffer := 25)
  	sturmBound := Floor((k/12)*Index(Gamma0(N)));
	return sturmBound + Buffer;
end function;
...
FindRelations := function(matrix)
	printf "finding relations %o x %o over %o ...", Nrows(matrix), Ncols(matrix), BaseRing(matrix);

	time kernelOfMatrix := Kernel(matrix);
	relations := [Eltseq(v) : v in Basis(kernelOfMatrix)];
	print "done findindinv relations";
	return relations;
end function;
...
function GetMonomialsOfWeightedDegree(R, k)
	mons := MonomialsOfWeightedDegree(R,k);

	// Make sure sorted with largest first (for GB monomial order):
	Sort(~mons);
	mons := Reverse(IndexedSetToSequence(mons));
	return mons;
end function;
...
ProduceMonomials := function(GenList,GenWeights, k, R, GBMons)
	//R<[x]> := PolynomialRing(Rationals(),GenWeights);

	FormalMonomials := GetMonomialsOfWeightedDegree(R,k);

	// Remove monomials which are divisible by any monomial m in GBMons
	// (because we are constructing a homogeneous GB and they will be
	// redundant since eliminated by m)
	for L in GBMons do
	    if L cmpne 0 then
		for m in L do
		    FormalMonomials := [
			u: u in FormalMonomials | not IsDivisibleBy(u, m)
		    ];
		end for;
	    end if;
	end for;
...
FindNewGenerators := function(N,k,EvaluatedMonomials,NumberOfRelations)
	printf "FindNewGenerators(%o, %o, %o, %o)\n...", N, k, [1..#EvaluatedMonomials], NumberOfRelations;
	prec := MinimumPrecision(N,k);
...
VectorInTermsOfBasis := function(N,k,genList,genWeights,inputVector)
	...
	R<[x]> := PolynomialRing(Rationals(),genWeights);
	evaluatedMonomials,formalMonomials := ProduceMonomials(genList,genWeights,k, R, []);
	...
	// Find the relation that doesn’t have a zero in last slot
...
ConstructGeneratorsAndRelations := function(N, MaxWeight)
  ...
  LowestWeight := SmallestWeight(N, MaxWeight);
	...
	RelMons := [* 0: i in [1 .. MaxWeight] *];
	GBMons := RelMons;

	function get_lens(M)
	    R := [L cmpeq 0 select 0 else #L: L in M];
	    while #R gt 0 and R[#R] eq 0 do
		Prune(~R);
	    end while;
	    return R;
	end function;
	...
	for i := (LowestWeight div 2 + 1) to (MaxWeight div 2) do
		k := 2*i;
		printf " k = %o, #RelMons: %o, #GBMons: %o\n",
		    k, get_lens(RelMons), get_lens(GBMons);

		Gens[k] := [];
		GenList := GenList cat [* f : f in Gens[k - 2]*];
		R<[x]> := PolynomialRing(Rationals(),GenWeights);

		procedure lift_mons(~list)
		    for i := 1 to #list do
			mons := list[i];
			if mons cmpeq 0 or IsNull(mons) then continue; end if;
			oldR := Universe(mons);
			h := hom<oldR -> R | [R.j: j in [1 .. Rank(oldR)]]>;
			list[i] := [h(m): m in mons];
		    end for;
		end procedure;
		...
		EvaluatedMonomials,NewMons := ProduceMonomials(GenList, GenWeights, k, R, GBMons);
		RelMons[k] := NewMons;

		if #EvaluatedMonomials ne 0 then
			prec := MinimumPrecision(N,k);
			printf "Build matrix...";
			time matCombos := Matrix([AbsEltseq(qExpansion(mon,prec)) : mon in EvaluatedMonomials]);
			...
			GBMons[k] := [R |
			    NewMons[rep{j: j in [1 .. #NewMons] | v[j] ne 0}]:
				v in NewRelations
			];
		else
			NumNewRelations := 0;
	...
	return Gens,GenList,GenWeights,Rels,RelWeights,RelMons;
end function;
...
MakePolyList := function(Rels,RelWeights,RelMons,GradedRing)
	PolynomialList := [];
	for i := 1 to #Rels do
		CurrentRelationWeight := RelWeights[i];
		CurrentRelation := Rels[i];
		Monomials := RelMons[CurrentRelationWeight];
		CurrentRelationPolynomial := &+[Monomials[j]*CurrentRelation[j] : j in [1..#CurrentRelation]];
...
// Output: R/I
ConstructRingAndIdeal := function(GenWeights,Rels,RelWeights,RelMons,N)
	R<[x]> := PolynomialRing(Rationals(),GenWeights);
	PolynomialList := MakePolyList(Rels,RelWeights,RelMons,R);
...
CanonicalRing := function(N : MaxWeight := 12)
	...
	gens,genList,genWeights,rels,relWeights,RelMons := ConstructGeneratorsAndRelations(N,MaxWeight);
	R,I,MGamma := ConstructRingAndIdeal(genWeights,rels,relWeights,RelMons,N);
